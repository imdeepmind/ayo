...
export default function StorageSettings() {
    ...
    const validateProviders = useCallback((): boolean => {
        ...
        for (const p of providers) {
            ...
            if (seen.has(key)) {
                const label =
                    p.type === 'azure' ? 'containerName' : 'bucketName';
                if (!newErrors[p.id]) newErrors[p.id] = {};
                newErrors[p.id][label] = `Duplicate: another ${providerLabel(p.type)} provider already uses this ${p.type === 'azure' ? 'container' : 'bucket'}`;
                valid = false;
                // Also mark the original
                const origId = seen.get(key)!;
                if (!newErrors[origId]) newErrors[origId] = {};
                newErrors[origId][label] = `Duplicate: another ${providerLabel(p.type)} provider already uses this ${p.type === 'azure' ? 'container' : 'bucket'}`;
            } else {
                seen.set(key, p.id);
            }
        }
